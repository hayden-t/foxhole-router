        ...
        static void Main(string[] args)
        {
            ...
            foreach (var current_feature in from feature in Source["features"] where feature["geometry"].HasValues select (feature, geometry: feature["geometry"]))
                current_feature.geometry["coordinates"] = JArray.FromObject(
                    from coordinateset in current_feature.geometry["coordinates"]
                    select JArray.FromObject( coordinateset.Select( (coordinate_value, index) => (index % 2 == 0 ? 128.0 : -128.0) + Math.Round(double.Parse((string)coordinate_value) * 128.0 / 20037508.3427892439067364, 4))));
