...
download()
{
		...
		#wget -qO - "https://war-service-live.foxholeservices.com/api/worldconquest/maps/"  | jq -r '.[]|.'`; do
                ...
                wget -qO - "https://war-service-live.foxholeservices.com/api/worldconquest/maps/$f/static" | jq "[.mapTextItems[] | if .mapMarkerType==\"Major\" then {\"key\":.text, \"value\": {region: \"$f\", x:(256+(((.x*46.54545454545455)+$offsety)-23.27272727272727)), y:(-256+((((1-.y)*40.30954606705751)+$offsetx)-20.15477303352875))}} else empty end]"
        done
        echo "]"
