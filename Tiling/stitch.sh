...
for g in $(ls *.png); do
	...
	if ! [ -z "$offsetx" ]; then
		if ! [ -z "$offsety" ]; then
			convert "$g" \( alphamask-0.png  -colorspace gray \) -alpha off -compose copy_opacity -composite "$g"
